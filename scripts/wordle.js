...
document.addEventListener('DOMContentLoaded', () => {
    // Generate 6x5 Wordle board
    generateWordleBoard(6, 5);

    // Only enable first row
...
// Create Wordle board by generating input cells
...
// Enable first input cell of row
function enableRow(rowIndex) {
    ...
    rowInputs.forEach(currentInput => {
        // On new character input, move focus to next input cell in the same row
        currentInput.addEventListener('input', () => {
            currentInput.value = currentInput.value.toLowerCase();
            moveFocusToNextInput(currentInput, rowInputs);
        })
        // On enter keydown, check if row is full then verify answer
        currentInput.addEventListener('keydown', (event) => {
            if (event.key === 'Enter') {
                checkIfRowIsFull(currentRowIndex);
            }
        })
        // On backspace keydown, move focus to previous input cell in the same row
        currentInput.addEventListener('keydown', (event) => {
...
// Move focus to previous input cell in the same row
...
// Move focus to the next input cell in the same row
...
// Check if all inputs in the row is filled then verify answer
function checkIfRowIsFull(rowIndex) {
    ...
    if (isFull) {
        ...
        if (isAnswerCorrect) {
            ...
            isGameOver = true;
            disableAllInputs();
            setTimeout(displayResult(resultModal, true), 1000);
            toggleButton(isGameOver);
        }
        else if (row.nextElementSibling) {
            // guess is wrong but there are empty rows remaining
            isGameOver = false;
            currentRowIndex += 1;
            enableRow(currentRowIndex)
        } else {
            // guess is wrong and there are no more empty rows
            isGameOver = true;
            disableAllInputs();
            setTimeout(displayResult(resultModal, false), 1000);
            toggleButton(isGameOver);
        }
    }
}

// Check each letter in the row, change color of each input cell according to result
...
// Reset word, board, and DOM elements
...
// Display result and hidden word
function displayResult(resultModal, isWinner) {
    ...
    resultMessage.innerHTML = 'Hidden Word: ' + correctAnswer.toUpperCase();
...
// Reset input cells of Wordle board
function clearWordleBoard() {
